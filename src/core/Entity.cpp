...
namespace inferno {
    ...
    bool Entity::has_tag(const std::string &tag) const {
        return tags_property.get().contains(tag);
    }

    void Entity::add_tag(const std::string &tag) {
        auto tags = tags_property.get();
        tags.insert(tag);
        tags_property.set(tags);
    }
    ...
    std::set<std::string> Entity::get_tags() const {
        return tags_property.get();
    }

    void Entity::set_tags(const std::set<std::string> &tags) {
        tags_property.set(tags);
    }

    std::vector<std::shared_ptr<Component>> Entity::get_components() const {
        return components_;
    }
    ...
    bool Entity::has_component(Component *const component) const {
        const auto found_component =
            std::ranges::find_if(components_, [&component](const std::shared_ptr<Component> &element) {
    ...
    void Entity::add_components(const std::vector<std::shared_ptr<Component>> &components) {
    ...
    void Entity::remove_component(Component *component) {
        if (const auto found_component = std::ranges::find_if(components_,
    ...
    void Entity::remove_components(const std::vector<std::shared_ptr<Component>> &components) {
        for (const auto &component : components) {
    ...
    void Entity::remove_pending_components_() {
        for (const std::set components_to_remove(components_to_remove_.begin(), components_to_remove_.end());
