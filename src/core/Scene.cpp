...
namespace inferno {
    ...
    std::vector<std::shared_ptr<Entity>> Scene::find_entities_by_name(const std::string &name) const {
    ...
    std::vector<std::shared_ptr<Entity>> Scene::find_entities_by_tag(const std::string &tag) const {
        std::vector<std::shared_ptr<Entity>> result;
        for (const auto &entity : entities_) {
            if (entity->has_tag(tag)) {
                result.push_back(entity);
    ...
    bool Scene::has_entity(Entity *entity) const {
        const auto found_entity = std::ranges::find_if(entities_, [&entity](const std::shared_ptr<Entity> &element) {
    ...
    void Scene::remove_entity(Entity *entity) {
        if (const auto found_entity = std::ranges::find_if(entities_,
    ...
    void Scene::remove_entities(const std::vector<std::shared_ptr<Entity>> &entities) {
        for (const auto &entity : entities) {
