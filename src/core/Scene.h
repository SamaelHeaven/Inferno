...
namespace inferno {
    ...
    protected:
        ...
        [[nodiscard]] std::vector<std::shared_ptr<Entity>> find_entities_by_name(const std::string &name) const;

        template <typename T> [[nodiscard]] std::shared_ptr<T> find_entity_by_type() const {
        ...
        template <typename T> [[nodiscard]] std::vector<std::shared_ptr<T>> find_entities_by_type() const {
        ...
        [[nodiscard]] std::vector<std::shared_ptr<Entity>> find_entities_by_tag(const std::string &tag) const;
        ...
        void add_entities(std::vector<std::shared_ptr<Entity>> entities);

        void remove_entity(const std::shared_ptr<Entity> &entity);

        void remove_entity(Entity *entity);

        template <typename T> void remove_entities() {
            for (const auto &entity : entities_) {
        ...
        void remove_entities(const std::vector<std::shared_ptr<Entity>> &entities);
